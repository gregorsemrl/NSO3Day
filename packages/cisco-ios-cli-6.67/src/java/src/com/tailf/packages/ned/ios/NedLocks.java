        ...
        try {
            ...
            for (int loop = 1; true; loop++) {
                ...
                for (int n = 0; n < list.size(); n++) {
                    ...
                    traceVerbose(worker, "locks: checking "+path);
                    ...
                    if (!owner.maapiExists(worker, toTh, path)) {
                        owner.traceInfo(worker, "locks: deleting (missing in CDB) : " + path);
                        deleteList.add(id);
                        triggered.add(id);
                        continue;
                    }
                    ...
                    if (!owner.maapiExists(worker, fromTh, path)) {
                        owner.traceInfo(worker, "locks: ignored, created : " + path);
                        triggered.add(id);
                        continue;
                    }
                    ...
                    String from = owner.maapiGetConfig(worker, fromTh, path, 0);
                    traceDev(worker, "   from = "+stringQuote(from));
                    String to = owner.maapiGetConfig(worker, toTh, path, 0);
                    ...
                    if (doRelock && to.equals(from)) {
                        String lock = unlock.replace("no ", "");
