...
*/
    ...
    /**
     * Append config from additional show and CDB which should not in transaction-id
     * @param
     * @return
     * @throws Exception
     */
    private String getConfigOnly(NedWorker worker, String res) throws Exception {
        ...
        try {
            ...
            if (isOnlineDevice()) {
                ...
                last.append(getSnmpUser(worker, res, th));
            }
     ...
     * @throws Exception
     */
    private String getSnmpUser(NedWorker worker, String dump, int th) throws Exception {
        ...
        traceInfo(worker, "transformed <= inserted: "+stringQuote(sb.toString())+" from 'show snmp user'");
        return sb.toString();
    }
            ...
            for (int n = injectConfig.size()-1; n >= 0; n--) {
                ...
                if (entry[1] == null) {
                    ...
                    if (entry[3] != null && entry[3].startsWith("after")) {
                        // inject last
                        traceInfo(worker, "transformed <= injected: "+stringQuote(entry[2])+" last in config");
                        last.append(entry[2] + "\n");
                    } else {
                        // inject first [default]
                        traceInfo(worker, "transformed <= injected: "+stringQuote(entry[2])+" first in config");
    ...
    private void injectCachedExec(NedWorker worker, StringBuilder first, int th) {
        ...
        try {
            ...
            if (val != null) {
                ...
                traceInfo(worker, "SECRETS - transformed <= injected 'key config-key password-encrypt <HIDDEN>'");
        ...
        try {
            ...
            for (int n = 0; n < lines.length; n++) {
                if (lines[n].startsWith("no key config-key password-encrypt")) {
                    ...
                    exitConfig(worker, "key config-key");
                    return;
                }
            }
        } catch (Exception ignore) {
    ...
    private String outputBgpNbrRedeploy(NedWorker worker, String data, int toTh) {
        ...
        for (int n = 0; n < lines.length; n++) {
            ...
            //
            // router-bgp * - Trim BGP neighbor changes and redeploy with peer-group change
            //
            ...
            for (; n < lines.length; n++) {
                ...
                if (nbr == null) {
                    ...
                    sb.append(line+"\n");
                } else if (!line.startsWith(" "+nbr+"peer-group ")) {
                    // Trim all non peer-group 'router bgp * / * / neighbor *'
                    ...
                    final String bgpPath = path.substring(0, path.indexOf("/neighbor")+1);
                    final String nbrPath = bgpPath + "address-family/ipv4{unicast}/neighbor{"+addr+"}";
                    if (maapiExists(worker, toTh, nbrPath)) {
                        traceVerbose(worker, "BGP: address-family ipv4 unicast redeploy: "+line);
                        inject = maapiGetConfig(worker, toTh, nbrPath, 1);
                        addrsb.append(inject);
                    }
                }
            }
        }
    ...
    private String reorderOutput(NedWorker worker, String data) throws NedException {
        ...
        //
        // Pass 4 - NedDiff (last, to let user override ordering)
        //
    ...
    private boolean isCliError2(NedWorker worker, int cmd, String replyall, String reply, String line, String toptag) {
        ...
        if (reply.contains("Invalid input detected at")) {
            ...
            if (toptag.startsWith("domain ")) {
                traceDev(worker, "domain toptag");
            }
    ...
    private int intCmdCheckConfigTrace(NedWorker worker, File file, StringBuilder reply, StringBuilder uniqueSb)
        ...
        try {
            while (myReader.hasNextLine()) {
                ...
                if (line.startsWith("-- NED VERSION: ") && !line.contains(" cisco-ios ")) {
    ...
    /**
     * Get NedWorker if available (may not exist in close())
     * @param
     * @throws
     */
    private NedWorker getWorker() {
        NedWorker worker = null;
        try {
    ...
    private String maapiDecryptLine(NedWorker worker, String line) {
        ...
        while (m.find()) {
            ...
            try {
                ...
                traceInfo(worker, "INFO: Failed to Maapi.decrypt '"+password+"' : "+e.getMessage());
                return line;
            }
            m = p.matcher(line);
        }
